...
<script>
    ...
    <main id="js-page-content" role="main" class="page-content">
        <ol class="breadcrumb page-breadcrumb">
            ...
            <li class="breadcrumb-item">Kategori Unduhan</li>
            <li class="position-absolute pos-top pos-right d-none d-sm-block"><span>{{ date('d F Y')}}</span></li>
        </ol>
        <div class="subheader">
            <h1 class="subheader-title">
                <i class='subheader-icon fal fa-table'></i> Kategori Unduhan: <span class='fw-300'>Portal WebApp</span>
            </h1>
        </div>
        <div class="row">
            <div class="col-xl-12">
                <div id="panel-1" class="panel">
                    <div class="panel-hdr">
                        <h2>
                            Tabel <span class="fw-300"><i>Kategori Unduhan</i></span>
                        </h2>
                    ...
                    <div class="panel-container show">
                        <div class="panel-content">
                            ...
                            <div class="modal fade" id="default-example-modal" tabindex="-1" role="dialog" aria-hidden="true">
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content">
                                        ...
                                        <form action="{{ route('katunduhan.store') }}" method="POST" accept-charset="utf-8" class="was-validated" enctype="multipart/form-data">
                                            {{ csrf_field() }}
                                            {{ method_field('POST') }}
                                            ...
                                            <div class="form-group">
                                                <label class="form-label" for="namaundh">Nama Kategori</label>
                                                <input type="text" name="namaundh"  id="namaundh" class="form-control is-valid" required="" >
                            ...
                            <table id="dt-basic-example" class="table table-bordered table-hover table-striped w-100">
                                ...
                                <tbody>
                                    @foreach($katunduhan as $undh)
                                        <tr>
                                            <td align="center">{{ $no++ }}</td>
                                            <td align="justify">{{ $undh->namaundh }} </td>
                                            <td align="justify">{{ $undh->keterangan }} </td>
                                            <td align="center">
                                                <form action="{{route('katunduhan.destroy', $undh)}}" method="post" id="delete_data_{{ $undh->id }}">
                                                {{ csrf_field() }}                          
                                                {{ method_field('DELETE') }}
                                                    <a href="{{ route('katunduhan.edit', $undh) }}">
                                                    ...
                                                    @if(auth()->user()->role == 'Ngadimin')
                                                        <button type="button" class="btn btn-sm btn-outline-danger" data-toggle="tooltip" data-placement="top" title="" data-original-title="Delete" onclick="hapus('{{ $undh->id }}')">
