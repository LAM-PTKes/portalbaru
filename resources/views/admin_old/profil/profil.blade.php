...
<script>
    ...
    <main id="js-page-content" role="main" class="page-content">
        <ol class="breadcrumb page-breadcrumb">
            ...
            <li class="breadcrumb-item">Profil LAM-PTKes</li>
            <li class="position-absolute pos-top pos-right d-none d-sm-block"><span>{{ date('d F Y')}}</span></li>
        </ol>
        <div class="subheader">
            <h1 class="subheader-title">
                <i class='subheader-icon fal fa-table'></i> Profil LAM-PTKes: <span class='fw-300'>Portal WebApp</span>
            </h1>
        </div>
        <div class="row">
            <div class="col-xl-12">
                <div id="panel-1" class="panel">
                    <div class="panel-hdr">
                        <h2>
                            Tabel <span class="fw-300"><i>Profil LAM-PTKes</i></span>
                        </h2>
                    ...
                    <div class="panel-container show">
                        <div class="panel-content">
                            ...
                            @if(auth()->user()->role == 'Ngadimin' || auth()->user()->role == 'Sekretariat')
                                <a href="{{ route('profil.create') }}" title="">
                            ...
                            <table id="dt-basic-example" class="table table-bordered table-hover table-striped w-100">
                                <thead class="bg-primary-600">
                                    <tr>
                                        ...
                                        <th><center>Nama Profil</center></th>
                                ...
                                <tbody>
                                    @foreach($profil as $prf)
                                        <tr>
                                            ...
                                            <td align="center">
                                                <form action="{{route('profil.destroy', $prf)}}" method="post" id="delete_data_{{ $prf->id }}">
                                                {{ csrf_field() }}                          
                                                {{ method_field('DELETE') }}
                                                    <a href="{{ route('profil.edit', $prf) }}">
                                                    ...
                                                    @if(auth()->user()->role == 'Ngadimin')
                                                        <button type="button" class="btn btn-sm btn-outline-danger" data-toggle="tooltip" data-placement="bottom" title="" data-original-title="Delete" onclick="hapus('{{ $prf->id }}')">
                                                            <i class="fas fa-trash-alt fa-2x"></i> 
                                                        </button>
                                                    @endif
                                                </form>
                                            </td>
                                            <td align="justify">{{ $prf->nprofil }} </td>
                                            <td align="justify">{!! $prf->isi_profil !!} </td>
                                            <td align="center">{{ $prf->katbahasa->namakbhs }} </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
