    ...
    <main id="js-page-content" role="main" class="page-content">
        ...
        <div class="subheader">
            <h1 class="subheader-title">
                <i class='subheader-icon fal fa-table'></i> Newsletter: <span class='fw-300'>Portal WebApp</span>
            </h1>
        </div>
        <div class="row">
            <div class="col-xl-12">
                <div id="panel-1" class="panel">
                    <div class="panel-hdr">
                        <h2>
                            Tabel <span class="fw-300"><i>Newsletter</i></span>
                        </h2>
                    ...
                    <div class="panel-container show">
                        <div class="panel-content">
                            ...
                            @if(auth()->user()->role == 'Ngadimin' || auth()->user()->role == 'RnD' || auth()->user()->role == 'SPMI' || auth()->user()->role == 'Sekretariat')
                                <a href="{{ route('nletter.create') }}" title="">
                            ...
                            <table id="dt-basic-example" class="table table-bordered table-hover table-striped w-100">
                                ...
                                <tbody>
                                    @foreach($newsletter as $nlt)
                                        <tr>
                                            <td align="center">{{ $no++ }}</td>
                                            <td align="center">
                                                <form action="{{route('nletter.destroy', $nlt)}}" method="post" id="delete_data_{{ $nlt->id }}">
                                                {{ csrf_field() }}
                                                {{ method_field('DELETE') }}
                                                    <a href="{{ route('nletter.edit', $nlt) }}">
                                                    ...
                                                    @if(auth()->user()->role == 'Ngadimin')
                                                        <button type="button" class="btn btn-sm btn-outline-danger" data-toggle="tooltip" data-placement="bottom" title="" data-original-title="Delete" onclick="hapus('{{ $nlt->id }}')">
                                            ...
                                            <td align="center">
                                                <a href="{{ route('secure.document.folder', ['folder' => 'newsletter', 'filename' => $nlt->gambar]) }}" title="" target="blank">
                                            ...
                                            <td align="center">{{ $nlt->katbahasa->namakbhs }} </td>
                                            <td align="justify">{{ $nlt->judul }} </td>
                                            <td align="center">{{ $nlt->publikasi }} </td>
                                            <td align="justify">{!! $nlt->deskripsi !!} </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
