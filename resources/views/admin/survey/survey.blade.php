    ...
    <main id="js-page-content" role="main" class="page-content">
        ...
        <div class="subheader">
            <h1 class="subheader-title">
                <i class='subheader-icon fal fa-table'></i> Survey: <span class='fw-300'>Portal WebApp</span>
            </h1>
        </div>
        <div class="row">
            <div class="col-xl-12">
                <div id="panel-1" class="panel">
                    <div class="panel-hdr">
                        <h2>
                            Tabel <span class="fw-300"><i>Survey</i></span>
                        </h2>
                    ...
                    <div class="panel-container show">
                        <div class="panel-content">
                            ...
                            @if(auth()->user()->role == 'Ngadimin' || auth()->user()->role == 'RnD' || auth()->user()->role == 'SPMI' || auth()->user()->role == 'Sekretariat')
                                <a href="{{ route('survey.create') }}" title="">
                            ...
                            <table id="dt-basic-example" class="table table-bordered table-hover table-striped w-100">
                                <thead class="bg-primary-600">
                                    <tr>
                                        ...
                                        <th><center>Waktu Penutupan</center></th>
                                        <th><center>Link Kuesioner</center></th>
                                ...
                                <tbody>
                                    @foreach($survey as $srv)
                                        <tr>
                                            ...
                                            <td align="center">
                                                <form action="{{route('survey.destroy', $srv)}}" method="post" id="delete_data_{{ $srv->id }}">
                                                {{ csrf_field() }}                          
                                                {{ method_field('DELETE') }}
                                                    <a href="{{ route('survey.edit', $srv) }}">
                                                    ...
                                                    @if(auth()->user()->role == 'Ngadimin')
                                                        <button type="button" class="btn btn-sm btn-outline-danger" data-toggle="tooltip" data-placement="bottom" title="" data-original-title="Delete" onclick="hapus('{{ $srv->id }}')">
                                                            <i class="fas fa-trash-alt fa-2x"></i> 
                                                        </button>
                                                    @endif
                                                </form>
                                            </td>
                                            <td align="center">{{ $srv->katbahasa->namakbhs }} </td>
                                            <td align="justify">{{ $srv->judul }} </td>
                                            <td align="justify">{{ $srv->tujuan }} </td>
                                            <td align="justify">{{ $srv->metode }} </td>
                                            <td align="justify">{{ $srv->responden }} </td>
                                            <td align="center">{{ date('d-m-Y', strtotime($srv->waktu_pelaksanaan)) }} </td>
                                            <td align="center">{{ date('d-m-Y', strtotime($srv->tgl_tutup)) }} </td>
                                            <td align="justify">{{ $srv->link }} </td>
                                            <td align="justify">{{ $srv->publikasi }} </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
