    ...
    <main id="js-page-content" role="main" class="page-content">
        ...
        <div class="subheader">
            <h1 class="subheader-title">
                <i class='subheader-icon fal fa-table'></i> Jurnal: <span class='fw-300'>Portal WebApp</span>
            </h1>
        </div>
        <div class="row">
            <div class="col-xl-12">
                <div id="panel-1" class="panel">
                    <div class="panel-hdr">
                        <h2>
                            Tabel <span class="fw-300"><i>Jurnal</i></span>
                        </h2>
                    ...
                    <div class="panel-container show">
                        <div class="panel-content">
                            ...
                            @if(auth()->user()->role == 'Ngadimin' || auth()->user()->role == 'RnD' || auth()->user()->role == 'SPMI' || auth()->user()->role == 'Sekretariat')
                                <a href="{{ route('jurnal.create') }}" title="">
                            ...
                            <table id="dt-basic-example" class="table table-bordered table-hover table-striped w-100">
                                <thead class="bg-primary-600">
                                    <tr>
                                        ...
                                        <th><center>File Jurnal</center></th>
                                ...
                                <tbody>
                                    @foreach($jurnal as $jnl)
                                        <tr>
                                            ...
                                            <td align="center">
                                                <form action="{{route('jurnal.destroy', $jnl)}}" method="post" id="delete_data_{{ $jnl->id }}">
                                                {{ csrf_field() }}
                                                {{ method_field('DELETE') }}
                                                    <a href="{{ route('jurnal.edit', $jnl) }}">
                                                    ...
                                                    @if(auth()->user()->role == 'Ngadimin')
                                                        <button type="button" class="btn btn-sm btn-outline-danger" data-toggle="tooltip" data-placement="bottom" title="" data-original-title="Delete" onclick="hapus('{{ $jnl->id }}')">
                                            ...
                                            <td align="center">
                                                <a href="{{ route('secure.document.folder', ['folder' => 'jurnal', 'filename' => $jnl->file_jurnal]) }}" title="" target="blank">
                                            ...
                                            <td align="center">{{ $jnl->katbahasa->namakbhs }} </td>
                                            <td align="justify">{{ $jnl->judul_jurnal }} </td>
                                            <td align="justify">{{ $jnl->nama_penulis }} </td>
                                            <td align="justify">{{ $jnl->kata_kunci }} </td>
                                            <td align="justify">{!! $jnl->abstrak !!} </td>
                                            <td align="center">{{ $jnl->publikasi }} </td>
                                            <td align="center">{{ $jnl->views }} </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
