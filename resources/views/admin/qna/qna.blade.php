...
<script>
    ...
    <main id="js-page-content" role="main" class="page-content">
        <ol class="breadcrumb page-breadcrumb">
            ...
            <li class="breadcrumb-item">Q&A Akreditasi</li>
            <li class="position-absolute pos-top pos-right d-none d-sm-block"><span>{{ date('d F Y')}}</span></li>
        </ol>
        <div class="subheader">
            <h1 class="subheader-title">
                <i class='subheader-icon fal fa-table'></i> Q&A Akreditasi: <span class='fw-300'>Portal WebApp</span>
            </h1>
        </div>
        <div class="row">
            <div class="col-xl-12">
                <div id="panel-1" class="panel">
                    <div class="panel-hdr">
                        <h2>
                            Tabel <span class="fw-300"><i>Q&A Akreditasi</i></span>
                        </h2>
                            ...
                            <table id="dt-basic-example" class="table table-bordered table-hover table-striped w-100">
                                <thead class="bg-primary-600">
                                    <tr>
                                        ...
                                        <th><center>Q&A Akreditasi</center></th>
                                ...
                                <tbody>
                                    @foreach($qnas as $qna)
                                        <tr>
                                            ...
                                            <td align="center">
                                                <form action="{{route('qna.destroy', $qna)}}" method="post" id="delete_data_{{ $qna->id }}">
                                                {{ csrf_field() }}                          
                                                {{ method_field('DELETE') }}
                                                    <a href="{{ route('qna.edit', $qna) }}">
                                                    ...
                                                    @if(auth()->user()->role == 'Ngadimin')
                                                        <button type="button" class="btn btn-sm btn-outline-danger" data-toggle="tooltip" data-placement="bottom" title="" data-original-title="Delete" onclick="hapus('{{ $qna->id }}')">
                                                            <i class="fas fa-trash-alt fa-2x"></i> 
                                                        </button>
                                                    @endif
                                                </form>
                                            </td>
                                            <td align="justify">{{ $qna->nqakre }} </td>
                                            <td align="justify">{!! $qna->deskripsi !!} </td>
                                            <td align="center">{{ $qna->katbahasa->namakbhs }} </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
