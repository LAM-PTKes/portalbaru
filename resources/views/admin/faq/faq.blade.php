    ...
    <main id="js-page-content" role="main" class="page-content">
        ...
        <div class="subheader">
            <h1 class="subheader-title">
                <i class='subheader-icon fal fa-table'></i> FAQ: <span class='fw-300'>Portal WebApp</span>
            </h1>
        </div>
        <div class="row">
            <div class="col-xl-12">
                <div id="panel-1" class="panel">
                    <div class="panel-hdr">
                        <h2>
                            Tabel <span class="fw-300"><i>FAQ</i></span>
                        </h2>
                                ...
                                <tbody>
                                    @foreach($faqs as $faq)
                                        <tr>
                                            ...
                                            <td align="center">
                                                <form action="{{route('faq.destroy', $faq)}}" method="post" id="delete_data_{{ $faq->id }}">
                                                {{ csrf_field() }}                          
                                                {{ method_field('DELETE') }}
                                                    ...
                                                    @if(auth()->user()->role == 'Ngadimin')
                                                        <button type="button" class="btn btn-sm btn-outline-danger" data-toggle="tooltip" data-placement="bottom" title="" data-original-title="Delete" onclick="hapus('{{ $faq->id }}')">
                                                            <i class="fas fa-trash-alt fa-2x"></i> 
                                                        </button>
                                                    @endif
                                                </form>
                                            </td>
                                            <td align="justify">{{ $faq->nfaq }} </td>
                                            <td align="justify">{{ $faq->pertanyaan }} </td>
                                            <td align="justify">{!! $faq->deskripsi !!} </td>
                                            <td align="center">{{ $faq->katbahasa->namakbhs }} </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
