...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $('#deskripsi').summernote('code', {!! json_encode($ifg->deskripsi) !!});
    });
    ...
    <main id="js-page-content" role="main" class="page-content">
        <ol class="breadcrumb page-breadcrumb">
            ...
            <li class="breadcrumb-item">Edit Info Grafis</li>
            <li class="position-absolute pos-top pos-right d-none d-sm-block"><span>{{ date('d F Y')}}</span></li>
        </ol>
        <div class="subheader">
            <h1 class="subheader-title">
                <i class='subheader-icon fal fa-table'></i> Edit Info Grafis: <span class='fw-300'></span>
            </h1>
        </div>
        <div class="row">
            <div class="col-xl-12">
                <div id="panel-1" class="panel">
                    <div class="panel-hdr">
                        <h2>
                            Form <span class="fw-300"><i>Edit Info Grafis</i></span>
                        </h2>
                    ...
                    <div class="panel-container show">
                        <div class="panel-content">
                            ...
                            <form method="post" action="{{ route('igrafis.update', $ifg) }}" class="was-validated" enctype="multipart/form-data" id="myForm">
                                {{ csrf_field() }}
                                {{ method_field('PATCH') }}

                                <div class="form-group">
                                    <label class="form-label" for="judul">Judul</label>
                                    <input type="text" name="judul" value="{{ $ifg->judul }}" id="judul" class="form-control is-valid" required="" >
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="katbahasa_id" id="katbahasa_id" class="form-control is-valid" required="">
                                        ...
                                        @foreach($katbhs as $bhs)
                                            <option @if($ifg->katbahasa_id == $bhs->id) value="{{ $bhs->id }}" selected="" @else value="{{ $bhs->id }}" @endif>{{ $bhs->namakbhs }}</option>
                                        @endforeach
                                    </select>
                                    <div class="invalid-feedback">Wajib Di isi</div>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="publikasi" id="publikasi" class="form-control is-valid" required="">
                                        <option value>Select</option>
                                        <option @if($ifg->publikasi == 'Ya') value="Ya" selected="" @else value="Ya" @endif>Publish</option>
                                        <option @if($ifg->publikasi == 'Tidak') value="Tidak" selected="" @else value="Tidak" @endif>Unpublish</option>
                                    </select>
                                    <div class="invalid-feedback">Wajib Di isi</div>
                                </div>
