...
<script type="text/javascript">
    ...
    <main id="js-page-content" role="main" class="page-content">
        <ol class="breadcrumb page-breadcrumb">
            ...
            <li class="breadcrumb-item">Edit Kontak Kami</li>
            <li class="position-absolute pos-top pos-right d-none d-sm-block"><span>{{ date('d F Y')}}</span></li>
        </ol>
        <div class="subheader">
            <h1 class="subheader-title">
                <i class='subheader-icon fal fa-table'></i> Edit Kontak Kami: <span class='fw-300'></span>
            </h1>
        </div>
        <div class="row">
            <div class="col-xl-12">
                <div id="panel-1" class="panel">
                    <div class="panel-hdr">
                        <h2>
                            Form <span class="fw-300"><i>Edit Kontak Kami</i></span>
                        </h2>
                    ...
                    <div class="panel-container show">
                        <div class="panel-content">
                            ...
                            <form method="post" action="{{ route('kontak.update', $ktk) }}" class="was-validated" enctype="multipart/form-data">
                                {{ csrf_field() }}
                                {{ method_field('PATCH') }}

                                <div class="form-group">
                                    <label class="form-label" for="ntentang">Judul</label>
                                    <input type="text" name="ntentang" value="{{ $ktk->ntentang }}" id="ntentang" class="form-control is-valid" required="" >
                                    <div class="invalid-feedback">Wajib Di isi</div>
                                </div>

                                <div class="form-group">
                                    <label class="form-label" for="tlp">No Telp 1</label>
                                    <input type="text" name="tlp" id="tlp" value="{{ $ktk->tlp }}" class="form-control is-valid" required="" >
                                    <div class="invalid-feedback">Wajib Di isi</div>
                                </div>

                                <div class="form-group">
                                    <label class="form-label" for="ponsel">No Telp 2</label>
                                    <input type="text" name="ponsel" id="ponsel" value="{{ $ktk->ponsel }}" class="form-control is-valid" required="" >
                                    <div class="invalid-feedback">Wajib Di isi</div>
                                </div>

                                <div class="form-group">
                                    <label class="form-label" for="kantor">No Telp 3</label>
                                    <input type="text" name="kantor" value="{{ $ktk->kantor }}" id="kantor" class="form-control is-valid" required="" >
                                    <div class="invalid-feedback">Wajib Di isi</div>
                                </div>

                                <div class="form-group">
                                    <label class="form-label" for="email">Email</label>
                                    <input type="email" name="email" value="{{ $ktk->email }}" id="email" class="form-control is-valid" required="" >
                                    <div class="invalid-feedback">Wajib Di isi</div>
                                </div>

                                <div class="form-group">
                                    <label class="form-label" for="jam_kerja">Jam Operasional Kantor</label>
                                    <textarea name="jam_kerja" id="jam_kerja" class="form-control is-valid" rows="5" required="">{{ $ktk->jam_kerja }}</textarea>
                                    <div class="invalid-feedback">Wajib Di isi</div>
                                </div>

                                <div class="form-group">
                                    <label class="form-label" for="alamat">Alamat Kantor</label>
                                    <textarea name="alamat" id="alamat" class="form-control is-valid" rows="5" required="">{{ $ktk->alamat }}</textarea>
                                    <div class="invalid-feedback">Wajib Di isi</div>
                                </div>

                                <div class="form-group">
                                    <label class="form-label" for="link">Link Google Map</label>
                                    <textarea name="link" id="link" class="form-control is-valid" rows="5" required="">{{ $ktk->link }}</textarea>
                                    <div class="invalid-feedback">Wajib Di isi</div>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="katbahasa" id="katbahasa" class="form-control is-valid" required="">
                                        ...
                                        @foreach($katbhs as $bhs)
                                            <option @if($ktk->katbahasa_id == $bhs->id)
