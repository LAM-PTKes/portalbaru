...
<script>
    ...
    <main id="js-page-content" role="main" class="page-content">
        <ol class="breadcrumb page-breadcrumb">
            ...
            <li class="breadcrumb-item">Kategori Bahasa</li>
            <li class="position-absolute pos-top pos-right d-none d-sm-block"><span>{{ date('d F Y')}}</span></li>
        </ol>
        <div class="subheader">
            <h1 class="subheader-title">
                <i class='subheader-icon fal fa-table'></i> Kategori Bahasa: <span class='fw-300'>Portal WebApp</span>
            </h1>
        </div>
        <div class="row">
            <div class="col-xl-12">
                <div id="panel-1" class="panel">
                    <div class="panel-hdr">
                        <h2>
                            Tabel <span class="fw-300"><i>Kategori Bahasa</i></span>
                        </h2>
                            ...
                            <div class="modal fade" id="default-example-modal" tabindex="-1" role="dialog" aria-hidden="true">
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content">
                                        ...
                                        <form action="{{ route('bahasa.store') }}" method="POST" accept-charset="utf-8" class="was-validated" enctype="multipart/form-data">
                                            {{ csrf_field() }}
                                            {{ method_field('POST') }}
                                            ...
                                            <div class="form-group">
                                                <label class="form-label" for="namakbhs">Nama Kategori</label>
                                                <input type="text" name="namakbhs"  id="namakbhs" class="form-control is-valid" required="" >
                                ...
                                <tbody>
                                    @foreach($bahasa as $bhs)
                                        <tr>
                                            ...
                                            <td align="justify">{{ $bhs->namakbhs }} </td>
                                            <td align="justify">{{ $bhs->keterangan }} </td>
                                            <td align="center">
                                                <form action="{{route('bahasa.destroy', $bhs)}}" method="post" id="delete_data_{{ $bhs->id }}">
                                                {{ csrf_field() }}                          
                                                {{ method_field('DELETE') }}
                                                    <a href="{{ route('bahasa.edit', $bhs) }}">
                                                    ...
                                                    @if(auth()->user()->role == 'Ngadimin')
                                                        <button type="button" class="btn btn-sm btn-outline-danger" data-toggle="tooltip" data-placement="top" title="" data-original-title="Delete" onclick="hapus('{{ $bhs->id }}')">
