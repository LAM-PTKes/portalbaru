@extends('awal.template.app')
@section('title', 'Unduh Instrumen APS Kualitaif Unggul')
@section('content')

    <div class="header-base">
        <div class="container">
            <div class="row">
                <div class="col-md-9">
                    <div class="title-base text-left">
                        <h1>Unduh Instrumen APS Kualitatif Unggul</h1>
                    </div>
                </div>
                <div class="col-md-3">
                    <ol class="breadcrumb b white">
                        <li><a href="{{ url('/') }}">Beranda</a></li>
                        <li><a href="#">Halaman</a></li>
                        <li class="active">Instrumen APS Kualitatif Unggul</li>
                    </ol>
                </div>
            </div>
        </div>
    </div>
    <div class="section-empty section-item">
        <div class="container content">
            <div class="row">
                <div class="col-12">
                    <div class="card-box table-responsive">

                        <table id="key-table" class="table table-bordered dt-responsive nowrap"
                            style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                            <thead>
                                <tr>
                                    <th>No</th>
                                    <th>Instrumen APS Kualitatif Unggul</th>
                                    {{-- <th>Tanggal Unggah</th> --}}
                                    <th>Program Studi Pengguna Instrumen</th>
                                    <th>Jenjang</th>
                                    <th>Bidang Ilmu</th>
                                    <th>Deskripsi</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($unduhans as $v => $unduhan)
                                    <tr>
                                        <td align="center">
                                            {{ $no++ }}
                                        </td>
                                        <td align="">
                                            <a href="{{ route('secure.document.folder', ['folder' => 'unduhan', 'filename' => $unduhan->nama_file]) }}" target="blank">
                                                <button type="button" class="anima-button btn-sm btn">
                                                    <i class="fa fa-download"></i>
                                                    {{ $unduhan->judul }}
                                                </button>
                                            </a>
                                            {{-- </td>
                                        <td>
                                            {{ $unduhan->updated_at->format('d/m/Y') }}
                                        </td> --}}
                                        <td>
                                            {{ $unduhan->pengguna_instrumen }}
                                        </td>
                                        <td>
                                            {{ $unduhan->jenjang }}
                                        </td>
                                        <td>
                                            {{ $unduhan->bidang_ilmu }}
                                        </td>
                                        <td>
                                            {!! $unduhan->deskripsi !!}
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                    </div>
                </div>
            </div> <!-- end row -->
        </div>
    </div>

@endsection
